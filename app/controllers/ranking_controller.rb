	...
	def show
		...
		if params[:id] == "2014"	
			@year = Stat.where("year = ?", 2014)
				@teamOrder = Stat.group("id", "team1").where("year = ?", 2014).order("team1")
				@currentData = @teamOrder.where("week = ?", @week)
			@teamValues = @currentData.pluck("Season2dateMean", "Season2dateOFFMean", "Season2dateDEFMean")
			@teamCurrent = Stat.where("year = ?", 2014).where("week = ?", @week).pluck("CurrentMeanTrend")	
			@dataValues = Stat.order("team1").where("year = ?", 2014).where("week = ?", @week).pluck("short1", "Season2dateMean", "CurrentMeanTrend", "Season2dateOFFMean", "CurrentOFFMeanTrend", "Season2dateDEFMean", "CurrentDEFMeanTrend")	
		#2013
